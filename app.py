...
with tab1:
    ...
    with col_a:
        ...
        fig.add_vline(x=3.3, line_dash="dot", line_color="#f0b429", annotation_text="World Avg 3.3%",
        ...
        fig.update_xaxis(title="GDP Growth (%)")
...
with tab2:
    ...
    with col3:
        ...
        fig_sec = go.Figure(go.Bar(
            ...
            orientation='h', marker_color=sector_sorted['color'],
    ...
    with col4:
        ...
        for status, group in supply_chain_data.groupby('Status'):
            fig_sc.add_trace(go.Scatter(
                ...
                marker=dict(size=group['Oil_Flow_MBPD']*1.5, color=colors_sc[status], opacity=0.85,
...
with tab3:
    ...
    with col1:
        ...
        fig_scatter.add_vrect(x0=7, x1=10, fillcolor="#ef444410", line_width=0,
                             annotation_text="High Geo Risk Zone", annotation_font_color="#ef4444",
    ...
    with col2:
        ...
        fig_go.add_trace(go.Scatter(x=gold_oil_months, y=gold_prices, name='Gold ($/oz)',
            line=dict(color='#f0b429', width=2.5), mode='lines+markers', marker=dict(size=8)), secondary_y=False)
        fig_go.add_trace(go.Scatter(x=gold_oil_months, y=oil_prices_go, name='Brent ($/bbl)',
            line=dict(color='#ef4444', width=2.5, dash='dash'), mode='lines+markers', marker=dict(size=8)), secondary_y=True)
        fig_go.update_layout(template=TEMPLATE, title="Gold vs Brent: Safe Haven Correlation",
                            height=200, legend=dict(orientation='h', y=1.08))
        fig_go.update_yaxes(title_text="Gold ($/oz)", secondary_y=False, title_font=dict(color='#f0b429'))
        fig_go.update_yaxes(title_text="Brent ($/bbl)", secondary_y=True, title_font=dict(color='#ef4444'))
...
with tab4:
    ...
    with col3:
        ...
        fig_vuln.add_trace(go.Scatter(
            ...
            marker=dict(size=country_vuln['GDP_Size']*8+12,
                       color=[risk_colors[r] for r in country_vuln['Risk_Score']],
                       opacity=0.8, line=dict(color='#0a0e1a', width=2)),
        ))
        fig_vuln.add_hrect(y0=60, y1=100, fillcolor='rgba(239,68,68,0.05)',
                          annotation_text="High Energy Import Risk", annotation_font_color='#ef4444',
                          annotation_font_size=9)
